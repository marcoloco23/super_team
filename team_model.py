...
for game_id in tqdm(game_ids):
    ...
    team_game_performances = team_performances[
        team_performances.GAME_ID == game_id
    ].drop_duplicates()
    ...
    for i in [
        team_1_player_performances[:team_size],
        team_2_player_performances[:team_size],
    ]:
...
targets = (
    pd.concat(target_list)
    .fillna(0)
    .reset_index(drop=True)
    .drop(["MIN", "PACE_PER40", "PACE", "E_PACE", "PTS", "PLUS_MINUS"], axis=1)
)
...
print(np.mean((predictions - test_labels) ** 2, axis=0).mean())
